...
const willValidateEmailAndPassword = async (email, password) => {
  let isValid;
  try {
    isValid = await willValidateEmail(email);
    isValid = isValid && (await willValidatePassword(password));
  } catch (error) {
    console.log("email validation failure " + error);
  }
...
const _createNewUserData = (email, password, token) =>
  _withHashedPassword(
    {
      email,
      name: email.split("@")[0],
    ...
    if (user) {
      ...
      switch (user.status) {
        case "WAIT_FOR_EMAIL_VERIFICATION":
          throw require("./errors").WAIT_FOR_EMAIL_VERIFICATION_ERROR;
        case "VERIFIED_BY_EMAIL":
          throw require("./errors").EMAIL_ALREADY_USE_ERROR;
      }
    } else {
...
const reset_password_by_token = (req, res) => {
  (async () => {
    ...
    const isValid = await willValidatePassword(password).catch(err =>
      res.json({ errors: [err.message] })
    );
    if (!isValid) {
      return res.json({ errors: ["token-invalid"] });
    }

    let user = await NAP.User
      .findOne({ token })
