...
const willSendVerification = async ({
  ...
  guard(
    { mailgun_api_key },
    "Required : mailgun_api_key, Missing .env MAILGUN_API_KEY?"
  );
  guard(
    { mailgun_domain },
    "Required : mailgun_domain, Missing .env MAILGUN_DOMAIN?"
  );
  guard({ email });
  ...
  // Send
  return await postmarkClient.messages.create(mailgun_domain, data);
};
